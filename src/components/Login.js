...
export default function Login() {
  ...
  async function submitHandler() {
    ...
    if (!validatePassword(password)) {
      setError(
        "Password should contain minimum 8 characters with atleast 1 Uppercase and 1 special character"
      );
      return;
    }
  ...
  function validatePassword(inputPasswd) {
    var validCheck = /^(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,20}$/;
    if (inputPasswd.match(validCheck)) {
      return true;
    } else {
      return false;
    }
